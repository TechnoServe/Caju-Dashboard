...
def getDepartmentName(code):
    ...
    return departments[(code[0:3].upper())]
        ...
        try:
            if items != 0:
                ...
                try:
                    ...
                    department_name = benin_yield.department
                    yield_pred_plant = int(tree_ha_pred_plant * dept_yield_ha[department_name])
                    surface_areaP = round(benin_yield.surface_area, 1)
                    total_yieldP = int(round(benin_yield.total_yield_kg))
                    yield_haP = int(total_yieldP / surface_areaP)
                    num_treeP = int(benin_yield.total_number_trees)
                    yield_treeP = int(round(total_yieldP / num_treeP))
                    nameP = benin_yield.owner_first_name + ' ' + benin_yield.owner_last_name
                    village = benin_yield.village
                except Exception:
                    department_name = getDepartmentName(code_2)
                    yield_pred_plant = int(tree_ha_pred_plant * dept_yield_ha[department_name])
                    surface_areaP = "NA"
                    total_yieldP = 0
                    yield_haP = "NA"
